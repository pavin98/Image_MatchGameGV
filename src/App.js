...
class App extends Component {
  ...
  updateImageList = () => {
    const randomNum = Math.floor(Math.random() * (imagesList.length - 1))
    const imageDetail = imagesList[randomNum]
    this.setState({imageDetails: imageDetail})
  }

  tabButtonElement = tabId => {
    this.setState({selectedButton: tabId})
  }
  ...
  render() {
    const {selectedButton, score, timer, imageDetails, isActive} = this.state
    const filteredList = imagesList.filter(
      eachList => eachList.category === selectedButton,
    )
    return (
      <div>
              ...
              <ul className="tabItem-list">
                {tabsList.map(eachItem => (
                  <TabItem
                    key={eachItem.tabId}
                    itemlist={eachItem}
                    updateTab={this.tabButtonElement}
                    CheckDet={selectedButton}
                  />
                ))}
              </ul>
